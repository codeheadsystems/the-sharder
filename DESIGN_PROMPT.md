# Design Prompt: `sharder`, a Language-Agnostic Sharding and Routing Library

## Your role

You are the architect for a new open-source library called `sharder`. Your job in this task is to
produce the design and normative specification, not an implementation. The spec must be precise
enough that two independent teams, working in two different languages, can implement it and produce
byte-identical routing decisions for the same inputs.

Read this whole prompt before starting, then read `docs/maintain/style.md`, which governs every
document you write. Where this prompt asks you to make a decision, make it, and record the
alternatives you rejected in a decision record. Where you genuinely cannot decide without input, put
the question in the open-questions register with a recommended default rather than silently
assuming.

## Documentation style

`docs/maintain/style.md` is binding on every file you produce under `docs/`. Read it before writing
a line. It is not a suggestion and it is not negotiable against your own habits; where your instinct
and the guide disagree, the guide wins.

The rules that will fight you hardest on a design document, and how they resolve here:

- Third person, present tense, about the library. Not about the reader, not about you, not about
  the document. A design document that says "we chose rendezvous hashing because we wanted" is
  wrong; "the library places keys by rendezvous hashing" is right.
- Bold marks a defined term on first use and nothing else. A specification is dense and the urge to
  bold the important clause is constant. Resist it. Position carries emphasis; typography does not.
- No capitalised stress. The one exception is the RFC 2119 vocabulary (`MUST`, `MUST NOT`, `SHOULD`,
  `SHOULD NOT`, `MAY`) inside `docs/design/10-specification.md`, where those words carry their
  RFC 2119 meaning. Every other document says "refuses" rather than "MUST refuse".
- No justification in the reference documents. This prompt repeatedly asks you to justify a
  decision, state a trade-off, and name prior art. All of that lands in the decision records under
  `docs/design/adr/`, which the style guide carves out for exactly this purpose. The overview and
  the specification state what the library does and link to the ADR; they do not argue for it, do
  not defend it against imagined objections, and do not certify how carefully it was considered.
- No em dashes anywhere. Serial commas. British spelling in prose, source spelling for identifiers.
  Prose wrapped at 100 columns. Noun-phrase headings under eight words, with no verbs of judgement.

When you report back at the end of the task, your assessment of the design's weak points goes in the
open-questions register and in your reply, not in the prose of the documents themselves.

## What the library is for

`sharder` answers one question well: given a key and a view of the world, which resources should
handle it, in what order, and what happens when those resources are unavailable?

It must serve at least these three concrete use cases, and the design must be validated against all
three:
...
1. Storage nodes, Dynamo-style. A keyspace is partitioned across storage nodes with N replicas per
   ...
   leave, and data has to move when they do.
2. Customer and tenant routing to services. Requests carrying a tenant identifier route to a service
   ...
   be no replication at all, just an ordered failover pool.
3. Cache and shard-affinity routing. Sticky routing where losing a node disturbs as few keys as
   possible, and where a briefly stale view of the topology must not be catastrophic.

These three pull in different directions. Where they conflict, the overview says what the design
does about it and the ADR says why, rather than asserting that one design serves all three.
...
- Failover: node health states, how an unavailable node is skipped, how far down the preference list
  to walk, and what the caller is told when the list runs out.
...
- Transport, RPC, serialisation of caller payloads, connection pooling, and wire-level retries.
- Storage engines, replication of data content, conflict resolution, vector clocks.
- Service discovery mechanics.

## Topology ownership

Topology is supplied through a pluggable provider SPI, not owned by the library.
...
- Reference providers that ship in core: in-memory and static file. Adapters for ZooKeeper, etcd,
  Consul, DynamoDB or a control-plane API are third-party and must be writable without forking the
  library. Demonstrate this by sketching one such adapter against the SPI.
...
For each of these, pick an answer, state the trade-off in an ADR, and record the rejected
alternatives there. The overview and specification carry the decision; the ADR carries the argument.
...
1. Which placement strategies ship in core. Evaluate at minimum: consistent hashing with virtual
   ...
   serve all three.
2. Whether the strategy is a pluggable interface, a closed set, or both. Define the interface either
   way.
...
10. Read routing against write routing: whether they share a preference list, how "prefer a replica
    in the local availability zone for reads" is expressed, and how that interacts with consistency
    expectations.
11. Retry and exhaustion: how many nodes deep the caller walks, who decides, and the exact result
    when the list is exhausted. Retry amplification is addressed.
...
22. Observability: required metrics with names and label sets, required events, and an explain API
    that answers why a given key routed where it did. Hot-shard and key-skew detection belong here.
23. The configuration surface and its defaults. Every default is defensible for a first-time
    integrator who reads none of the documentation.
...
Write these into the repository at these exact paths. `docs/maintain/style.md` already exists and is
...
2. `docs/design/10-specification.md`. The normative specification. Every requirement carries a
   ...
   language-neutral pseudocode or IDL, never in Java. The reader is writing an implementation, and
   this is the document the conformance suite tests against. Requirement identifiers are the join
   key between the specification and the tests.
...
- A small, sharp core with well-defined extension points, rather than a framework. An integrator who
  ...
  state machine.
- Prior art is named in the ADRs, with what is borrowed: Dynamo preference lists, Cassandra vnodes
  and token ranges, Redis Cluster hash slots, Kafka partition assignment, Envoy ring hash, Maglev
  and outlier detection, Ceph CRUSH failure domains, and HRW hashing. Note where the design
  diverges.
...
change and rebalance lifecycle, then conformance. Write each ADR as the decision is made, not at the
end.

When you finish, state in your reply which parts of the design you are confident in, which parts are
thinner than you would like, and what you would want reviewed first. That assessment belongs in the
reply and in `docs/design/90-open-questions.md`, not in the prose of the other documents.
