...
specification, the topology document format and its JSON Schema, the decision records, the Java
binding design, and a conformance suite of language-neutral data files computed by a Python
reference implementation.

Two kinds of change are possible today: a change to a Markdown document, and a change to the
conformance suite through its generator. There is no Gradle build, no `check` task, and no Java
source; [`docs/design/99-roadmap.md`](docs/design/99-roadmap.md) gives the staging.
...
The planned Java build is Gradle with the Kotlin DSL, group `com.codeheadsystems`, artifacts
`sharder-api`, `sharder-core`, `sharder-migrate`, `sharder-provider-file`, `sharder-conformance`,
`sharder-conformance-vectors`, `sharder-bom`, and `sharder-bench`, with `buildSrc` carrying
`verifyDocLinks`, `verifyDocStyle`, and `verifyUnsignedComparisons`. None of it exists yet, so every
rule those tasks would enforce is enforced at review.
