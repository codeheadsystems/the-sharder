...
public interface PreparedPlacement {
    ...
    /** The candidate ordering for a routing key over an eligible node set. */
    List<NodeId> candidates(byte[] routingKey, EligibleSet eligible);
    ...
    /** The candidate ordering of a shard, under {@code PLACE-033}. */
    List<NodeId> candidatesForShard(String shard, EligibleSet eligible);
